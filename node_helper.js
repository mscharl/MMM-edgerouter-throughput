...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    module.exports = NodeHelper.create({
        ...
        _fetchData: function () {
            var _this = this;
            var sshClients = this.sshClients;
            var promises = Object
                .entries(sshClients)
                ...
                .map(function (_a) {
                var gateway = _a[0], client = _a[1];
                return client.exec('/sbin/ifstat --json').then(function (result) { return ({
                    gateway: gateway,
                    result: result,
                }); });
            })
                // Parse and normalize the result.
                .map(function (promise) { return promise.then(function (_a) {
                var gateway = _a.gateway, result = _a.result;
                var output = JSON.parse(result);
                var data = output[Object.keys(output)[0]];
                return {
                    gateway: gateway,
                    data: data,
                };
            ...
            Promise.all(promises)
                ...
                .then(function (results) { return results.reduce(function (payload, _a) {
                var _b;
                var gateway = _a.gateway, data = _a.data;
                return __assign({}, payload, (_b = {}, _b[gateway] = data, _b));
            }, {}); })
                // Send the payload via socket notifications.
                .then(function (payload) {
                _this.sendSocketNotification(ModuleNotification.THROUGHPUT, payload);
            });
        },
        /**
